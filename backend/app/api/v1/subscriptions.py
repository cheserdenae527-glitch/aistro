...
async def refresh_sub(sub_id: str, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    api = _get_xhs_api()
    # 用户资料
    success, msg, raw = api.get_user_info(sub.xhs_user_id)
    if success and raw:
        d = raw.get('data', raw)
        bi = d.get('basic_info', {})
    ...
    # 笔记数
    success2, msg2, notes_data = api.get_user_all_notes(f'https://www.xiaohongshu.com/user/profile/{sub.xhs_user_id}')
    ...
    result = SubscriptionResponse.model_validate(sub).model_dump()
    result["notes"] = _notes[:50]
    return result
...
async def get_sub_notes(sub_id: str, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    from crawler.processor import normalize_note
    api = _get_xhs_api()
    success, msg, raw_notes = api.get_user_all_notes(f'https://www.xiaohongshu.com/user/profile/{sub.xhs_user_id}')
